...
1. Один агент — просто
2. 10 агентов — всё ещё просто
3. 100 агентов — начинаются сюрпризы
4. 1000 агентов — добро пожаловать в инфраструктурный ад
5. Как мы из него выбрались
...
- Цикл: **сбор данных → обучение → обмер модели** — нужен throughput
- 1000 параллельных агентов ≈ минимальная планка для быстрых итераций
- Цель: модель с SoTA-результатами на кодовых бенчмарках

---

### Зачем масштабировать?

> **СПОЙЛЕР:** Побеждает не тот, у кого лучше модель, а тот, у кого быстрее итерации
```
┌──────────────────┬──────────────────┬──────────────────┬──────────────────┐
│                  │     БЫЛО         │    СЕЙЧАС        │      ЦЕЛЬ        │
│                  │   (до платформы) │                  │  (ближайшая)     │
├──────────────────┼──────────────────┼──────────────────┼──────────────────┤
│                  │                  │                  │                  │
│  Вычисления      │  2 сервера       │  Kubernetes      │  Эластичный      │
│  (CPU)           │  ручной Docker   │  11+ нод         │  кластер,        │
│                  │                  │  ресурсы на лету │  авто-скейлинг   │
│                  │                  │                  │                  │
├──────────────────┼──────────────────┼──────────────────┼──────────────────┤
│                  │                  │                  │                  │
│  Инференс        │  8 GPU           │  96 GPU          │  200+ GPU        │
│  (GPU)           │  1 машина        │  (12 × 8)        │  объединение     │
│                  │                  │  единая точка    │  кластеров       │
│                  │                  │  доступа         │                  │
│                  │                  │                  │                  │
├──────────────────┼──────────────────┼──────────────────┼──────────────────┤
│                  │                  │                  │                  │
│  Параллельные    │  20              │  200             │  1 000           │
│  агенты          │  на 1 сервере    │  стабильно       │                  │
│                  │                  │                  │                  │
├──────────────────┼──────────────────┼──────────────────┼──────────────────┤
│                  │                  │                  │                  │
│  Замер           │  ~9~14 часов     │  ~2 часа         │  < 1 час         │
│  бенчмарка       │                  │                  │                  │
│  (500 задач)     │                  │                  │                  │
│                  │                  │                  │                  │
├──────────────────┼──────────────────┼──────────────────┼──────────────────┤
│                  │                  │                  │                  │
│  Генерация       │  ~100 дней       │  ~10 дней        │  ~дни            │
│  100к решений    │                  │                  │                  │
│  задач           │                  │                  │                  │
│                  │                  │                  │                  │
└──────────────────┴──────────────────┴──────────────────┴──────────────────┘
...
Масштабирование → ускорение цикла → больше экспериментов → лучше модель

Было: 100 дней сбор данных + 7 дней обучение + день обмеров ~= 107 дней
Сейчас: 10 дней на сбор данных + 7 дней обучение + 2 часа обмеров ~= 17 дней (ускорение ~6 раз)
Стремимся: 2 дней на сбор данных + 7 дней обучение + 2 часа обмеров ~= 9 дней (ускорение ~11 раз)

За теже самое время работы можно будет проверить гораздо больше гипотез

---

## Часть 1: Один агент — просто (слайды 6–15)
...
[СКРИНШОТ: простая схема — один контейнер с агентом]
...
- **Автоматическая верификация**: тесты проходят/не проходят — без людей
- Это делает масштабирование возможным и осмысленным

---

### Анатомия одного запуска
...
│  └── eval.sh (тесты)       │
├─────────────────────────────┤
│ Агент (код + LLM-клиент)   │
│  ├── MCP (bash в контейнере)│
...
1. **Подготовка**: достать репозиторий, установить зависимости
2. **Решение**: агент «думает» и вносит изменения
3. **Проверка**: запуск тестов, сбор результатов

---

### Один агент — что нужно?

- Docker
- Модель (API или локальный vLLM)
- Агентный фреймворк
- ~30 минут на задачу
