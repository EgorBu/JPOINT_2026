...
### Анатомия одного запуска # TODO - удалить
...
### Анатомия запуска - три фазы задачи

1. **Подготовка**: клонировать репозиторий, установить зависимости
2. **Решение**: запускается LLM + агентный фреймворк -> и агент вносит изменения
3. **Проверка**: запуск тестов, сбор результатов

---

### Анатомия запуска - три фазы задачи # TODO

1. **Подготовка**: достать репозиторий, установить зависимости - надо кэшировать, вносить в образ все, чтобы не качать по 100500 раз зависимости
2. **Решение**: запускается llm + агентный фреймворк - и агент вносит изменения
3. **Проверка**: запуск тестов, сбор результатов
...
- Изоляция - Docker/bubblewrap/etc
- Модель (API или локальный vLLM)
- Агентный фреймворк
- ~30 минут на задачу

**Просто, правда?** (вопрос залу - кто пытался запускать агента? Кто пытался написать своего агента?)
...
### С чем придется столкнуться (Engineering Tax)
...
*   **State Persistence**: Агент должен уметь «засыпать» и «просыпаться» (Resume session) без потери цепочки рассуждений.
*   **Streaming UI**: Рендеринг `thinking` блоков, `diff` в реальном времени и интерактивных диалогов в терминале (React Ink / Blessed).
