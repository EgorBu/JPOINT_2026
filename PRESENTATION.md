...
### Когда речь идет о десятках задач, то проблем нет - всё работает хорошо
### Но когда задач сотни тысяч , начинаются проблемы
...
# Зачем нужны бенчмарки для кодовых агентов?

## Как устроен типичный SWE-бенчмарк:
1. **Задача**: issue + ожидаемое поведение + критерий успеха
2. **Окружение**: фиксированный commit + зависимости + команда тестов
3. **Верификация**:  
   - до фикса часть тестов падает (Fail-to-Pass)  
   - после фикса они проходят, а остальные не ломаются (Pass-to-Pass)

## Простые задачи научились решать все, нужны сложные задачи
...
## Воронка задач SWE-smith

TODO Надо подумать нужна ли эта таблица, мб сократить

| Strategy | # Repos | # Candidates | # Instances | Yield Rate |
|---|---:|---:|---:|---:|
| Combine (file) | 124 | 6,020 | 5,865 | 97.43% |
| Combine (module) | 65 | 4,396 | 4,227 | 96.16% |
| LM (Modify) | 108 | 31,950 | 17,887 | 55.98% |
| LM (Rewrite) | 128 | 11,908 | 4,173 | 35.04% |
| PR Mirroring | 108 | 6,934 | 2,344 | 33.80% |
| Procedural (Class Rm Base) | 103 | 1,401 | 463 | 33.05% |
| Procedural (Class Rm Funcs) | 103 | 2,506 | 1,180 | 47.09% |
| Procedural (Class Shuffle Funcs) | 103 | 2,504 | 47 | 1.88% |
| Procedural (Ctrl Invert If) | 105 | 4,695 | 2,321 | 49.44% |
| Procedural (Ctrl Shuffle) | 104 | 9,055 | 4,015 | 44.34% |
| Procedural (Op Break Chains) | 71 | 747 | 225 | 30.12% |
| Procedural (Op Change Const) | 77 | 723 | 257 | 35.55% |
| Procedural (Op Change) | 81 | 1,507 | 450 | 29.86% |
| Procedural (Op Swap) | 87 | 2,141 | 483 | 22.56% |
| Procedural (Remove Assign) | 121 | 5,470 | 2,661 | 48.65% |
| Procedural (Remove Cond) | 120 | 5,288 | 2,311 | 43.70% |
| Procedural (Remove Loop) | 110 | 1,945 | 860 | 44.22% |
| Procedural (Remove Wrapper) | 80 | 884 | 368 | 41.63% |
| All | 129 | 100,074 | 50,137 | 50.10% |
...
- Позволят автоматизировать весь пайплайн
- Возможность маcштабирования
- Codex парадигма - дал задачу и забыл 
...
* Инфраструктурный ад
или
* вместо использования железа для результатов агенты конкурируют за CPU & RAM & DISC
...
* подсчитывать pass rate неудобно через парсинг S3
...
* каждый образ из внешней сети скачивается один раз
* внутренняя сеть обладает гораздо более высокими лимитами 
...
## 4 месяца 
* подняли кубер 
* сделали его стабильным
* Логи, мониторинг, автоматизация замеров

* Больше 100к задач прорешали агенты за последнюю неделю
